...
HcclResult CollReduceScatterMeshAivFor91093Executor::GetIfNeedAivBuffer(bool &needAivBuffer)
{
    // AIV通信需要AIV buffer
    needAivBuffer = true;
    HCCL_INFO("[CollReduceScatterMeshAivFor91093Executor][GetIfNeedAivBuffer]tag[%s] needAivBuffer is [%u].",
...
u32 CollReduceScatterMeshAivFor91093Executor::CalBlockDim(u32 rankSize, u64 dataSize, HcclCMDType cmdType)
{
    ...
    HCCL_INFO("[CollReduceScatterMeshAivFor91093Executor][CalBlockDim] blockDim is set to [%u]", blockDim);
    return blockDim;
}
