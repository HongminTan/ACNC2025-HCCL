 ...
 */
#include "coll_all_gather_multi_stream_mesh_executor.h"

namespace hccl {
CollAllGatherMultiStreamMeshExecutor::CollAllGatherMultiStreamMeshExecutor(const HcclDispatcher dispatcher,
...
HcclResult CollAllGatherMultiStreamMeshExecutor::CalcStreamNum(u32& streamNum)
{
    u32 totalStreamNum = topoAttr_.deviceNumPerAggregation > 1U ? topoAttr_.deviceNumPerAggregation - 1U : 1U;
    streamNum = totalStreamNum - 1U;
    HCCL_INFO("[CollAllGatherMultiStreamMeshExecutor][CalcStreamNum] tag[%s] streamNum[%u]",
        tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}

HcclResult CollAllGatherMultiStreamMeshExecutor::CalcCommInfo(std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult CollAllGatherMultiStreamMeshExecutor::CalcLevel0CommInfo(TransportMemType inputType, TransportMemType outputType,
...
u64 CollAllGatherMultiStreamMeshExecutor::CalcLoopMaxCount(const u64 cclBuffSize, const u32 unitSize)
{
...
HcclResult CollAllGatherMultiStreamMeshExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    HCCL_CONFIG_INFO(HCCL_ALG, "[CollAllGatherMultiStreamMeshExecutor][KernelRun]allgather multi stream mesh start");
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAllGatherMultiStreamMeshExecutor][KernelRun]all gather multi stream mesh memcpy Failed, Offset[%llu], Size[%llu].",
    ...
    level0TempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(TemplateType::TEMPLATE_ALL_GATHER_MULTI_STREAM_MESH,
                                                                      dispatcher_);
    ...
    HCCL_INFO("all gather multi stream mesh level0 run success");
    ...
    HCCL_INFO("all gather multi stream mesh level1 run success");
    return HCCL_SUCCESS;
}

REGISTER_EXEC("AllGatherMultiStreamMeshExecutor", AllGatherMultiStreamMesh, CollAllGatherMultiStreamMeshExecutor);
} // namespace hccl
