...
#ifndef AIV_REDUCESCATTER_CROSSNODE_91093_BASE_H
#define AIV_REDUCESCATTER_CROSSNODE_91093_BASE_H

#include "aiv_communication_base.h"

using namespace AscendC;

class AivReduceScatterCrossNode91093Base {
public:
    __aicore__ inline AivReduceScatterCrossNode91093Base() {}

    template<typename T>
    __aicore__ inline void Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize, uint32_t baseFlagOffset,
        uint32_t reduceOp, uint64_t avgBufferCount, uint64_t len); // 单算子的init

    template<typename T>
    __aicore__ inline void Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize, uint32_t baseFlagOffset,
        uint32_t reduceOp, uint64_t len); // 图模式的init
    ...
    __aicore__ inline void SingleRecordBatchWaitCoreLevel(uint32_t flagOffset, int32_t curTag, bool isTheSingleCore);
...
__aicore__ inline uint64_t AivReduceScatterCrossNode91093Base::CeilDiv(uint64_t a, uint64_t b)
{
    if (b == 0) {
        return a;
    }
...
__aicore__ inline uint64_t AivReduceScatterCrossNode91093Base::CalActualCount(uint32_t sliceIdx, uint64_t sliceCount,
...
__aicore__ inline void AivReduceScatterCrossNode91093Base::CalCountAndBlockOffset(uint64_t len, uint32_t blockNumPerGroup, 
...
// 单算子的Init
template<typename T>
__aicore__ inline void AivReduceScatterCrossNode91093Base::Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize,
    uint32_t baseFlagOffset, uint32_t reduceOp, uint64_t avgBufferCount, uint64_t len)
{
    ...
    if (rankSize > HALF_MAX_BLOCK_DIM) {
        ...
        blockNumPerGroup = 1;
        if (len <= avgBufferCount) { // ccl够用，只需要搬一轮的情况
            countMid = 0;
            countTail = len;
        } else if (len % avgBufferCount == 0) { // ccl不够用，要搬多轮的情况1: 能整除
            countMid = avgBufferCount;
            countTail = avgBufferCount;
        } else { // ccl不够用，要搬多轮的情况2: 不能整除
            countMid = avgBufferCount;
            countTail = len % avgBufferCount;
        }
        blockOffsetMid = 0;
        blockOffsetTail = 0;
        flagOffsetInGroup = 0;
...
template<typename T>
__aicore__ inline void AivReduceScatterCrossNode91093Base::Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize,
    uint32_t baseFlagOffset, uint32_t reduceOp, uint64_t len)
{
    baseFlagOffset_ = baseFlagOffset;
    flagAddrSelf_ = buffOut0 + baseFlagOffset;
...
template<typename T>
__aicore__ inline void AivReduceScatterCrossNode91093Base::SetAtomicOp(uint32_t atomicOp)
{
    switch (atomicOp) {
...
template<typename T>
__aicore__ inline void AivReduceScatterCrossNode91093Base::DataCopyGM2UB(const LocalTensor<T>& dstLocal, const GlobalTensor<T>& srcGlobal,
    const uint32_t calCount)
{
...
template<typename T>
__aicore__ inline void AivReduceScatterCrossNode91093Base::DataCopyUB2GM(const GlobalTensor<T>& dstGlobal, const LocalTensor<T>& srcLocal,
    const uint32_t calCount)
{
    if ((calCount * sizeof(T)) % UB_ALIGN_SIZE == 0) {
...
template<typename T>
__aicore__ inline void AivReduceScatterCrossNode91093Base::CpGM2GM(__gm__ T *outputGM, __gm__ T *inputGM, uint64_t count)
{
...
template<typename T>
__aicore__ inline void AivReduceScatterCrossNode91093Base::CpGM2GMInlineReduce(__gm__ T *outputGM, __gm__ T *inputGM, uint64_t count, uint32_t atomicOp)
{
    GlobalTensor<T> inputGT;
    inputGT.SetGlobalBuffer(inputGM, count);
    GlobalTensor<T> outputGT;
...
template<HardEvent event> 
__aicore__ inline void AivReduceScatterCrossNode91093Base::SyncFunc() {
    int32_t eventID = static_cast<int32_t>(GetTPipePtr()->FetchEventID(event));
    SetFlag<event>(eventID);
    WaitFlag<event>(eventID);
}

__aicore__ inline void AivReduceScatterCrossNode91093Base::SingleRecordBatchWaitCoreLevel(uint32_t flagOffset, int32_t curTag, bool isTheSingleCore)
{
    ...
    // 写一个自己的flag，标志自己的那一片localcopy(input->output)完成
    if (isTheSingleCore) {
...
__aicore__ inline void AivReduceScatterCrossNode91093Base::BatchRecordWait(GM_ADDR* buffersOut, uint32_t flagOffset, int32_t curTag)
{
    // 写所有对端的flag
    localSetTensor.SetValue(0, curTag);
    GlobalTensor<int32_t> globalTag;
...
#endif  /* AIV_REDUCESCATTER_CROSSNODE_91093_BASE_H */