...
HcclResult ReduceScatterPlantLocalReduceCombine::LocalCopy()
{
    ...
    DeviceMem src;
    if (isUseCclIn_) {
        src = inputMem_.range(memBlockInfo_.userInputOffsets[localRank_], sliceSize);  // cclin
    } else {
    ...
    if (isLevel0LastRank_ && isLastBlockData(outputIndex) && !isLastRank(localRank_)) {
    ...
    const std::vector<LINK> &links)
{
    ...
    if (isLevel0LastRank_ && !isLastRank(round) && isLastBlockData(outputIndex)) {
    ...
    if (isLevel0LastRank_ && isLastBlockData(localOutputIndex) && !isLastRank(localRank_)) {
...
HcclResult ReduceScatterPlantLocalReduceCombine::RunLocalReduce()
{
    ...
    for (u32 round = 0; round < reduceStep; round++) {
        ...
        for (u32 offset = 0; offset <= tailIndex - headIndex; offset++) {   
            ...
            void *srcPtr;
            void *dstPtr;
            if (isLevel0LastRank_ && !isLastRank(localRank_) && isLastBlockData(inputIndex)) {
            ...
            if (isLevel0LastRank_ && !isLastRank(localRank_) && isLastBlockData(outputIndex)) {
