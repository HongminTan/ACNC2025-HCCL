...
#include "all_gather_multi_stream_mesh.h"
#include "externalinput_pub.h"
#include "alg_template_register.h"

namespace hccl {
AllGatherMultiStreamMesh::AllGatherMultiStreamMesh(const HcclDispatcher dispatcher)
    : AlgTemplateBase(dispatcher)
{}

AllGatherMultiStreamMesh::~AllGatherMultiStreamMesh() {}

HcclResult AllGatherMultiStreamMesh::Prepare(std::vector<Stream> &meshStreams,
...
HcclResult AllGatherMultiStreamMesh::Tx(const LINK &link, const Slice &txSlice, const Slice &dstSlice, Stream stream)
{
...
HcclResult AllGatherMultiStreamMesh::Rx(const LINK &link, const Slice &srcSlice, const Slice &rxSlice, Stream stream)
{
...
HcclResult AllGatherMultiStreamMesh::RunAllGather(const std::vector<LINK> &links, const std::vector<Slice> &outputSlices,
...
// allgather的入口函数
HcclResult AllGatherMultiStreamMesh::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK> &links)
{
    HcclResult ret = HCCL_SUCCESS;
    CHK_SMART_PTR_NULL(dispatcher_);
    CHK_PTR_NULL(stream_.ptr());
    HCCL_INFO("AllGatherMultiStreamMesh run: rank[%u] totalrank[%u] inputMem[%p] outputMem[%p] count[%llu]", 
    ...
    HCCL_INFO("AllGatherMultiStreamMesh finished: rank[%u]", rank);
    return HCCL_SUCCESS;
}
REGISTER_TEMPLATE(TemplateType::TEMPLATE_ALL_GATHER_MULTI_STREAM_MESH, AllGatherMultiStreamMesh);
} // namespace hccl
